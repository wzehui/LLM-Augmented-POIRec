...
DATASET_PATHS = {
    "meta+geo": "../yelp/embeddings/openai_augmented_dataset_training_meta+geo.pickle",
    "UGC": "../yelp/embeddings/openai_augmented_dataset_training_UGC.pickle",
}
...
def objective(trial):
    if model_class.__name__ == "GRURecWithEmbeddings":
        llm2grurec_config = {
            "N": 25,
            "activation": "relu",
            "emb_dim": trial.suggest_int("emb_dim", 256, 1024, step=64),
            "fit_batch_size": trial.suggest_int("fit_batch_size", 16, 128, step=16),
            "hidden_dim": trial.suggest_int("hidden_dim", 128, 2048, step=64),
            "optimizer_kwargs": {
