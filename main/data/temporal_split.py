...
class TemporalSplit(MECESplit):
    ...
    def __init__(
        ...
        if test_cutoff is not None:
            if isinstance(test_cutoff, float):
                self.test_cutoff: datetime = datetime.fromtimestamp(test_cutoff)
            elif isinstance(test_cutoff, datetime):
                self.test_cutoff: datetime = test_cutoff
            else:
                raise ValueError(
                    "The cutoff points must be either datetime objects or floats."
                )
